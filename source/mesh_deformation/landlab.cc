...
namespace aspect
{
  ...
  namespace MeshDeformation
  {
      ...
      if (this_rank_runs_landlab)
        {
          ...
          AssertDimension(current_solution_at_points[0].size(), n_solution_variables);
          AssertDimension(derived_quantities_at_points[0].size(), variable_names.size() - n_solution_variables);
    ...
    {
      // Default remains consistent with the melt extractor model default.
      this->melt_extractor_extraction_depth = 7000.0;
      ...
      // Read extraction depth from the Reactive Fluid Transport material model
      // so this parameter is defined in one place.
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Reactive Fluid Transport Model");
        {
          if (prm.get("Fluid-solid reaction scheme") == "melt extractor")
            {
              prm.enter_subsection("Melt extractor");
              {
                this->melt_extractor_extraction_depth = prm.get_double("Extraction depth");
              }
              prm.leave_subsection();
            }
        }
