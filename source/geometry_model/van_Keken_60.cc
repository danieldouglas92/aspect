...
#include <aspect/geometry_model/van_Keken_60.h>
...
namespace aspect
{
  namespace GeometryModel
  {
    ...
    create_coarse_mesh (parallel::distributed::Triangulation<2> &coarse_grid) const
    {
      ...
      const double angle = std::tan(60 * deg2rad);
      const std::vector<Point<2>> vertices =
      {
        //   0            1             2             3             4
        {0.0, 0.0}, {50e3/angle, 0.0}, {330e3/angle, 0.0}, {600e3/angle, 0.0}, {405e3, 0.0},
        //   5              6               7               8               9
        {0.0, 300e3}, {50e3/angle, 300e3}, {233e3/angle, 183e3}, {330e3/angle, 270e3}, {405e3, 270e3},
        //    10             11            12              13
        {50e3/angle, 550e3}, {405e3, 550e3}, {0.0, 600e3}, {405e3, 600e3}
      };
    ...
    template <int dim>
    Point<dim>
    vanKeken60<dim>::get_extents () const
    {
    ...
    template <int dim>
    Point<dim>
    vanKeken60<dim>::get_origin () const
    {
    ...
    template <int dim>
    double
    vanKeken60<dim>::depth(const Point<dim> &position) const
    {
    ...
    template <int dim>
    double
    vanKeken60<dim>::height_above_reference_surface(const Point<dim> &) const
    {
      return 0.0;
    }
    ...
    template <int dim>
    Point<dim>
    vanKeken60<dim>::representative_point(const double depth) const
    {
      Point<dim> p;
      p(dim-1) = depth;
      return p;
    ...
    template <int dim>
    double
    vanKeken60<dim>::maximal_depth() const
    {
      return 600e3;
    }
    ...
    template <int dim>
    bool
    vanKeken60<dim>::point_is_in_domain(const Point<dim> &) const
    {
      return true;
    }
    ...
    vanKeken60<dim>::natural_coordinate_system() const
    {
    ...
    template <int dim>
    void
    vanKeken60<dim>::declare_parameters (ParameterHandler &)
    {}
    ...
    template <int dim>
    void
    vanKeken60<dim>::parse_parameters (ParameterHandler &)
    {}
  }
}
...
namespace aspect
{
  namespace GeometryModel
  {
    ASPECT_REGISTER_GEOMETRY_MODEL(vanKeken60,
                                   "vanKeken60 box",
